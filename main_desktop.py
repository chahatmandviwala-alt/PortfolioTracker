...
def fetch_latest_fx(from_ccy: str, to_ccy: str) -> float:
    """
    Get current FX rate (from_ccy → to_ccy). Cached for 1 hour.
    Uses frankfurter 'latest' endpoint.
    """
    from_ccy = (from_ccy or "").upper()
    to_ccy = (to_ccy or "").upper()
...
with st.sidebar:
    ...
    hide_values = st.toggle("Hide Portfolio Values", value=True)
